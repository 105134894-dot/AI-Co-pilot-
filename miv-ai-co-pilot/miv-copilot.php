...
function miv_enqueue_copilot_assets() {
    ...
    wp_enqueue_script(
        ...
        array(), // add 'jquery' here if you ever need it
        '1.0',
        true // load in footer
    );

    // Pass config (like backend URL) to JS
    wp_localize_script(
        'miv-copilot-script',
        'MIV_WIDGET_CONFIG',
        array(
...
function miv_inject_copilot_widget() {
    $launcher_img = plugins_url('assets/img/miv-button.png', __FILE__); // adjust image name/path as needed
    ?>
    <div class="miv-widget-root" id="miv-widget-root">
        <button class="miv-launcher-btn" id="miv-launcher-btn" aria-label="Open MIV AI Co-Pilot">
            <img src="<?php echo esc_url($launcher_img); ?>" alt="MIV AI Co-Pilot" class="miv-launcher-img" />
        </button>
