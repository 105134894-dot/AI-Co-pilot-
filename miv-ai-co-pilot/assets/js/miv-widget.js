...
(function () {
  ...
  // Push the current state onto the history stack when navigating to a new state
  // unless the new state is identical to the current one.
  ...
  function renderIntentButtons() {
    ...
    INTENT_CATEGORIES.forEach(cat => {
      ...
      btn.addEventListener("click", () => {
        ...
        // Push current state so Back returns to fresh-start UI
        historyStack.push({ ...currentState });
  ...
  /* -----------------------------
     Rendering helpers
     - dedupe temp messages to avoid duplicates
  ----------------------------- */
  ...
  // Apply theme based on currentThemeIndex
   ...
   * opts.skipNavigate: if true, do not call navigateTo here (caller already navigated)
   */
  ...
  // Close entire chat when clicking outside root
  ...
  // Close accessibility panel when clicking outside it (but not closing chat)
  document.addEventListener("pointerdown", (e) => {
  ...
  // Cycle through 7 contrast themes
